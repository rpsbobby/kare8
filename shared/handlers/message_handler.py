...
class MessageHandler(Generic[T, V]):
    ...
    # --- Entry point ---
    def handle_message(
            self, message: Dict[str, Any], key: Optional[bytes] = None, headers: Optional[dict[str, str]] = None, ) -> None:
    ...
    def _handle_main_message(
            self, message: Dict[str, Any], key: Optional[bytes], headers: Optional[dict[str, str]], ) -> None:
    ...
    def _handle_dlq_message(self, message: Dict[str, Any], key: Optional[bytes], headers: Optional[dict[str, str]]) -> None:
        ...
        try:
            ...
            park_headers=self._merge_headers(headers, {
                "x-trace-id": dlq.trace_id, "x-attempts": str(dlq.attempts), "x-error-code": dlq.error_code,
                }, )
        ...
        try:
            ...
            fail_headers=self._merge_headers(headers, {
                "x-trace-id": dlq.trace_id, "x-attempts": str(dlq.attempts), "x-error-code": dlq.error_code,
                }, )
    ...
    def _produce(
            self, topic: str, payload: Dict[str, Any], key: Optional[bytes] = None, headers: Optional[dict[str, str]] = None, ) -> None:
        self.producer.produce(topic, payload, key=key, headers=headers)

    def _merge_headers(
            self, base: Optional[dict[str, str]], extra: dict[str, str], ) -> dict[str, str]:
        h=dict(base or {})
